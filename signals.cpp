...
//        }
        ...
        smash.jobsList->hasPipeInFg = false;
        return;
    }
...
//            smash.jobsList->removeTimeoutJob(smash.jobsList->timeoutJobs[i].id);
        ...
        if (job->command->getPID() != smash.smashPid){
//            cout << "timeout job id: " << smash.jobsList->timeoutJobs[i].id << " sleep: " << smash.jobsList->timeoutJobs[i].sleepTime << endl; //todo: debug
//            cout << "job id: " << job->jobID << " job cmd: " << job->command->cmd_line << endl;//todo: debug
            int sleepTime = smash.jobsList->timeoutJobs[i].sleepTime;
            if (now - job->timeStamp >= smash.jobsList->timeoutJobs[i].sleepTime){
                if (kill(job->command->getPID(), SIGKILL) != 0){
                    ...
                    cout << "smash: timeout " << sleepTime << " " << cmd_line1 << " timed out!" << endl;
                    smash.jobsList->removeTimeoutJob(smash.jobsList->timeoutJobs[i].id, smash.jobsList->timeoutJobs[i].pipe);
